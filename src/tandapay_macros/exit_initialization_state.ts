...
export async function exitInitializationState(params: ExitInitializationStateParams): Promise<ExitInitializationStateReturnType> {
  const {reader, membersToAdd} = params;
  
  const memberCount = Number(await reader.getCurrentMemberCount());
  if (memberCount + membersToAdd.length < InitializationStateConstants.minCommunitySizeToExit) {
    return {
