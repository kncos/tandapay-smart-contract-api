...
import { AbiEvent, Address, BlockNumber, GetEventArgs, GetLogsReturnType, Hash, Log } from "viem";
...
type RawEventName = Extract<(typeof TandaPayInfo.abi)[number], { type: "event"; }>["name"];
...
export const RawEventNameToAliasMapping: {[K in RawTandaPayEventName]: TandaPayEventAlias} = {
...
export const AliasToRawEventNameMapping: {[K in TandaPayEventAlias]: RawTandaPayEventName} = {
  addedToCommunity: "AddedToCommunity",
  additionalDayAdded: "AdditionalDayAdded",
...
export function tandaPayEventNameToAbiEvent(eventName: RawTandaPayEventName) {
  let [e] = TandaPayInfo.abi.filter(item => item.type === "event" && item.name === eventName);
  return e;
}

/** Accepts an array of raw TandaPay event names and returns an array of abi items for those events */
export function tandaPayEventNamesToAbiEvents(eventNames: RawTandaPayEventName[]) {
...
export function tandaPayEventAliasesToAbiEvents(aliases: TandaPayEventAlias[]) {
  return tandaPayEventNamesToAbiEvents(aliases.map(alias => AliasToRawEventNameMapping[alias]));
}

/** A type predicate that returns whether or not a given string is a raw TandaPay event name */
export function isRawTandaPayEventName(eventName: string): eventName is RawTandaPayEventName {
...
export function getRawTandaPayEventArgs(rawEventName: RawTandaPayEventName) {
  const event = TandaPayInfo.abi.filter(t => t.type === "event").find(e => e.name === rawEventName);
  if (!event)
    return [];
 ...
 */
export function toTandaPayLogs(logs: GetLogsReturnType<AbiEvent>): TandaPayLog[] {
  let tandaPayLogs: TandaPayLog[] = [];
  for (let l of logs) {
    // ensure that the eventName is a valid event on the TandaPay ABI
    if (!isRawTandaPayEventName(l.eventName))
      continue;

    // ensure that the args are the appropriate ones for this event
    if (!isValidEventArgs(l.eventName, l.args))
      continue;
