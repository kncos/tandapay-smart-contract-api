...
import { TandaPayRole, TandaPayState } from "../../src/contract_managers/types";
...
test("get to default state", async () => {
    ...
    const secretary = managersAndWallets.managers[0] as WriteableTandaPayManager<TandaPayRole.Secretary>;
    const members = managersAndWallets.managers.slice(1) as WriteableTandaPayManager<TandaPayRole.Member>[];
    ...
    for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < 5; j++) {
            ...
            await secretary.write.secretary.addMemberToCommunity(wallets[walletIndex].account.address);
            await secretary.write.secretary.assignMemberToSubgroup(wallets[walletIndex].account.address, BigInt(i+1));
        }
    }
