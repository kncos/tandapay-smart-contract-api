...
import { exitInitializationState } from "tandapay_macros/exit_initialization_state";
...
import { getInitializationStateInfo } from "tandapay_macros/information_retrieval/get_initialization_state_info";
...
describe('exit initialization state macro tests', () => {
  it('basic case, fresh community with no members, assigning members and specifying successors', async () => {
    const exitInitializationStateResult = exitInitializationState({
      newMembersToAdd: suite.accounts.map(acc => acc.address),
      totalCoverageWei: DEFAULT_COVERAGE_REQUIREMENT,
      secretarysuccessorsList: suite.accounts.map(acc => acc.address).slice(0,2),
      allMemberInfo: [],
      subgroupCount: 0,
    });
  ...
  it('see what information we can get', async () => {
    const initializationStateInfo = await getInitializationStateInfo(suite.secretary.read, suite.secretary.batchRead);
