...
describe('TandaPay Read Methods', () => {
  it('can get the payment address token', async () => {
    ...
    expect(paymentTokenAddr.toUpperCase()).toBe(defaultStateInfo.ftkAddress.toUpperCase());
  });
  ...
  it('get Member ID from address', async () => {
    const memberId = await manager.read.getMemberIdFromAddress(writeableClient.account.address);
  ...
  it('get member info from both address and id', async () => {
    const memberInfo1 = await manager.read.getMemberInfoFromAddress(writeableClient.account.address);
