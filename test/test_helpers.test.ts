import { advanceTime, deployFaucetToken, deployTandaPay, makeAccounts, makePublicClients, makeTestClient, makeWalletClients, spawnAnvil } from "./test_helpers";
...
describe("anvil and client connections", () => {
    ...
    test("can create wallet clients that connect to anvil", async () => {
        ...
        if(!wc1.account || !wc2.account)
            fail("wallet client has no account!");

        if (!wc1.chain || !wc2.chain)
            fail("wallet client has no chain!");
    ...
    test("can deploy faucet token & tandapay contracts", async () => {
        ...
        if (!receipt)
            fail("transaction receipt is null or undefined?");
